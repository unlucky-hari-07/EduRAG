...
Wraps the Sentence Transformers embedding model. Kept in its own module so
the embedding model can be swapped (via utils/config.py) without touching
...
def _get_model():
    """Lazily load the embedding model (heavy import, loaded once per session)."""
    global _model
    if _model is None:
